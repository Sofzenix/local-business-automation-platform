import { sendMessageWithSmsFallback } from "../services/whatsappService.js";
...
export async function handle(event, data) {
  switch (event) {
    /* -------------------------------
       INCOMING MESSAGE
    --------------------------------*/
    ...
    case "INCOMING_MESSAGE":
      ...
      if (newConversation) {
        ...
        if (welcomeTemplate) {
          ...
          try {
            await sendMessageWithSmsFallback(phone, messageText);
      ...
      if (orderItems) {
        ...
        try {
          await sendMessageWithSmsFallback(cleanOwnerPhone, ownerMessage);
          console.log("Owner WhatsApp message sent");
        } catch (error) {
          console.log("WhatsApp failed for owner. Sending SMS fallback...");

          try {
            await sendSMS(cleanOwnerPhone, ownerMessage);
    ...
    case "DAILY_SUMMARY":
      ...
      try {
        await sendMessageWithSmsFallback(cleanOwnerPhone, messageText);
        console.log("Daily summary sent successfully");
      } catch (error) {
        console.log("WhatsApp summary failed. Sending SMS fallback...");

        try {
          await sendSMS(cleanOwnerPhone, messageText);
        } catch (smsError) {
