...
export async function handle(event, data) {
  switch (event) {
    ...
    case "DAILY_SUMMARY":
      ...
      await sendMessageWithSmsFallback(phone, message);
    ...
    case "LOW_STOCK":
      ...
      try {
        await sendMessageWithSmsFallback(data.phone, lowStockMessage);
        console.log("Low stock alert sent");
      } catch (error) {
        console.log("WhatsApp failed. Sending SMS fallback...");

        try {
          await sendSMS(data.phone, lowStockMessage);
        } catch (smsError) {
    ...
    case "EXPIRY_ALERT":
      ...
      try {
        await sendMessageWithSmsFallback(data.phone, expiryMessage);
        console.log("Expiry alert sent");
      } catch (error) {
        console.log("WhatsApp failed. Sending SMS fallback...");

        try {
          await sendSMS(data.phone, expiryMessage);
        } catch (smsError) {
          console.log("SMS fallback also failed:", smsError.message);
        }
      }
